...
- **WHEN** a user passes an unknown preset identifier after `doc_doctor` is present in the built-in registry
...
- **WHEN** preset resolution for `doc_doctor` is evaluated from different current working directories
