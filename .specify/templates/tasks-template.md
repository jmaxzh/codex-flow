...
**Tests**: Include tasks that provide automated test evidence whenever behavior changes.
Tests are OPTIONAL only for documentation-only or non-behavioral work explicitly marked
as such in the feature specification.
...
- [ ] T004 Document affected contract/spec files under docs/specs/ (or mark no contract impact)
...
- [ ] T011 Define validation task list for quality gates: `ruff check`, `ruff format`,
      `basedpyright --warnings`, and `python3 -m pytest -q` (or justified scoped variants)
...
### Tests for User Story 1 (include for behavioral changes) ⚠️
...
### Tests for User Story 2 (include for behavioral changes) ⚠️
...
### Tests for User Story 3 (include for behavioral changes) ⚠️
...
- [ ] TXXX Run required quality gates and capture results in PR/summary
